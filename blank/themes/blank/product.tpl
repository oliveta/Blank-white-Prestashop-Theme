...
{literal}
$(function() {
$("input.add_to_cart").css('opacity',0);
})
{/literal}
//]]>
</script>
...
<div id="primary_block" class="clearfix grid_12">
	...
	<div id="pb-right-column">
		<!-- product img-->
			...
			<span id="view_full_size" class="view_full_size">&nbsp;</span>
			{/if}
		<div id="image-block">
		{if $have_image}
	...
	<div id="pb-left-column">
        ...
        {if isset($groups)}<h5>{l s='Personalize it !!'}</h5>{/if}
			...
			{if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
             <div id="global_price">
              <h5 class="cufon">{l s='Price and order'}</h5>
           ...
           <div{if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE} style="display: none;"{/if} id="add_it">
			...
			<span class="cufon size3 upper">{l s='Add to cart'}</span>
             <input type="submit" name="Submit" value="" class="add_to_cart" />
            </p>
           </div>
			...
			<p id="availability_statut"{if ($product->quantity == 0 && !$product->available_later) OR ($product->quantity != 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				...
				<span id="availability_value"{if $product->quantity == 0} class="warning-inline"{/if}>
					{if $product->quantity == 0}{if $allow_oosp}{$product->available_later}{else}{l s='This product is no longer in stock'}{/if}{else}
					
					
					{$product->available_now}{/if}
				</span>
			</p>
			<div id="availability_statut_1" {if ($product->quantity == 0) OR (!$product->available_for_order)  OR $PS_CATALOG_MODE} style="display: none;"{/if}>
			{$HOOK_PRODUCT_FOOTER}
...
<!-- description and features -->
	...
	{/if}
{if  $features || $HOOK_PRODUCT_TAB || $attachments}
...
<li><span class="alike">{l s='Accessories and related items'}</span></li>
</ul>
...
{assign var='accessoryPriceWithoutRedution' value=Product::getPriceStatic($accessory.id_product)}
			
			<p class="pprice">
			 <span>{displayWtPrice p=$accessory.price}</span>
			 
			  {if $accessoryPriceWithoutRedution>$accessory.price}
            <span id="old_price_home"  >
                  {if  !$priceDisplay || $priceDisplay <= 2}
                    {if $accessoryPriceWithoutRedution > $accessory.price}
                      <span id="" style="font-size:11px;
	color:#cc0000;
	text-decoration: line-through;
	font-weight:normal;">{convertPrice price=$accessoryPriceWithoutRedution}</span>
			  ...
			  <a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{l s='View'}">{l s='View'}</a> </p>
			  
              {if ($accessory.id_product.quantity > 0 OR $accessory.id_product.allow_oosp)}
			  <p><a class="ajax_add_to_cart_button" href="{$base_dir}cart.php?qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Add to cart'}">{l s='Add to cart'}</a></p>
