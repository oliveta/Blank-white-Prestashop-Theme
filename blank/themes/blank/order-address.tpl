...
<h1 class="nadpis" >{l s='Addresses'}</h1>
<div class="table-5">
{include file="$tpl_dir./errors.tpl"}
<form action="{$link->getPageLink($back_order_page, true)}" id="form_add" method="post">
{else}
...
{/if}
	<div class="addresses">
		...
		<div id="ordermsg">
			...
			<p class="textarea"><textarea cols="60" rows="6" name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea></p>
		</div>
		{/if}
	</div>
{if !$opc}
	<p class="cart_navigation submit">
		...
		<a class="continue upper cufon size3" id="form_submit" href="{$link->getPageLink('order.php', true)}?step=2{if $back}&amp;back={$back}{/if}" title="{l s='Next'}" >{l s='Next'} &raquo;</a>
		
		<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="exclusive hidden" />
