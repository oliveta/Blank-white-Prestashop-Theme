		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if($('#cart_block dt#cart_block_product_'+ domIdProduct ).length == 0)
				{
					...
					var cufonsmall='cufon';
					if($.browser.msie && parseInt($.browser.version, 10) <= 7) {
    cufonsmall='';
}   
						 content += '<span class="quantity-formated"><span class="quantity '+cufonsmall+'">' + this.quantity + '</span><span class="'+cufonsmall+'">x&nbsp;</span></span>';
						 var name = (this.name.length > 50 ? this.name.substring(0, 50) + '...' : this.name);
