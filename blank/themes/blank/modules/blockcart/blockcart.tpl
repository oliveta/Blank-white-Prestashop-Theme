...
<div id="cart_block" class="block exclusive">
	<h4 >
		<a href="{$link->getPageLink("$order_process.php", true)}" class="cufon size3 upper">{l s='Cart' mod='blockcart'}</a>
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				...
				<a class="" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}" title="{$product.name|escape:html:'UTF-8'}">
	...
	{/if}
		...
		<p id="cart-prices">
			<span class="floater">{l s='Shipping' mod='blockcart'}</span>
			<span id="cart_block_shipping_cost" class="price ajax_cart_shipping_cost">{$shipping_cost}</span>
			<br/>
			{if $show_wrapping}
				{assign var='blockcart_cart_flag' value='Cart::ONLY_WRAPPING'|constant}
				<span class="floater">{l s='Wrapping' mod='blockcart'}</span>
		...
		<p id="cart-buttons">
			...
			<a href="{$link->getPageLink("$order_process.php", true)}{if $order_process == 'order'}{/if}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if} cufon " title="{l s='Check out' mod='blockcart'}"><span class="size3 upper">{l s='Check out' mod='blockcart'}</span></a>
		</p>
