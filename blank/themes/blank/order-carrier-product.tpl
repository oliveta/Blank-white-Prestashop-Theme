...
<h5 class="cufon">{l s='Delivery methods'}</h5>
	<table id="table-5" {if ( !isset($carriers) || !$carriers || !count($carriers)) && empty($HOOK_EXTRACARRIER)}style="display:none;"{/if}>
		<thead>
			<tr>
				<th>{l s='Carrier'}</th>
		...
		{if isset($carriers)}
			{foreach from=$carriers item=carrier name=myLoop}
				...
				<tr id="carrier{$carrier.id_carrier|intval}" class="{if $smarty.foreach.myLoop.index % 2 == 0}carrier_odd{else}carrier_even{/if}">
					...
					<td >
						{if $carrier.price && $productPrice<$free}
							<span class="price">
...
<p id="availability_statut">
			{l s='Free shipping'}<b>{convertPriceWeb price=$free}</b>.
