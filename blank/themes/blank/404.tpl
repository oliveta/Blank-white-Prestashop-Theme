...
{capture name=path}{l s='Page not available'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<div class="table_block" style="margin-top:1.5em">

<h1>{l s='Page not available'}</h1>
<p>{l s='We\'re sorry, but the Web address you entered is no longer available'}</p>
<h3>{l s='To find a product, please type its name in the field below'}</h3>

<form action="{$link->getPageLink('search.php')}" method="post" class="std" id="forgot_password">
	<fieldset>
		<p class="text">
        ...
        <p class="submit_pwd">
			<input type="submit" name="Submit" value="OK" id="submitPassword" />
        </p>
	</fieldset>
</form>
